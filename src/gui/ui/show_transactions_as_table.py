...
class ShowTransaction:
    ...
    @classmethod
    def genaertion_table(cls, tree: ttk.Treeview, transactions: list, index_flitter: list=[3, 8, 9, 10]) -> ttk.Treeview:
    ...
    def show_transactions(cls, transactions: list, index_flitter: list=[3, 8, 9, 10]):
        ...
        tree = cls.genaertion_table(transactions=transactions, index_flitter=index_flitter, tree=tree)
        # tree["columns"] = heading_text
        # for index, item in enumerate(heading_text):
        #     tree.column(item, width=all_heading_width[index])
        #     tree.heading(item, text=item)
